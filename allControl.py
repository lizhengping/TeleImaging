...
class control:
    ...
    def JTDCOpen(self):
        os.chdir('C:\\lidar\\program\\Dataprocess\\JTDC\\run')
        shell_cmd = "run.bat"
    ...
    def SignalInit(self):
        ...
        # ID=getConfig(["SL_ID"])
        # CH1_V=getConfig(["SL_CH1_High","SPI_CH1_Low"])
        # CH1_F=getConfig(["SL_CH1_Frequency"])
        # CH2_V=getConfig(["SL_CH2_High","SPI_CH2_Low"])
    ...
    def take_photo(self):
        ...
        step=getConfig(['step'])
        pixelTime=getConfig(['pixelTime'])


        self.cam.tele1.point_At_uRad_Precise(state[0],state[1])
        self.cam.takePhoto(self.xSubPic,self.ySubPic,step,pixelTime)
